...
const addEmployee=async(e)=>{
  // e.preventDefault();
  const {name,email,mobileNo,designation,gender,course,img} = employee;
 try{
 let res = await axios.post("http://localhost:9000/create",employee)
  ...
  return (
    ...
    <Navbar/>
      <form style={{ margin: 'auto', marginLeft: '25vw', width: '80vw',marginTop:"3vw", }}>
        ...
        {/* Submit Button */}
        <button type="submit" className="btn btn-primary" style={{ marginLeft: '25vw', width: '10vw', marginTop: '4vw' }} onClick = {addEmployee}>Submit</button>
      </form>
    </>
  );
}
